...
struct HighOrderGodunovKernel {
    ...
    inline void operator()(std::size_t const faceIdx) {
        ...
        auto rhoELeft = ctx.rhoLeft[faceIdx] * (ctx.eLeft[faceIdx] + 0.5 * ctx.uuLeft[faceIdx]);

        // cell-centered total energy density on the right of the face
        auto rhoERight = ctx.rhoRight[faceIdx] * (ctx.eRight[faceIdx] + 0.5 * ctx.uuRight[faceIdx]);

        // cell-centered u.B on the left of the face
        auto uBLeft = ctx.uLeft[faceIdx] * ctx.bXLeft[faceIdx] +
                      ctx.vLeft[faceIdx] * ctx.bYLeft[faceIdx] +
                      ctx.wLeft[faceIdx] * ctx.bZLeft[faceIdx];
        
        // cell-centered u.B on the right of the face
        auto uBRight = ctx.uRight[faceIdx] * ctx.bXRight[faceIdx] +
        ...
        ctx.rhoFlux[faceIdx] = 0.5 * ctx.faceArea[faceIdx] *
                               (ctx.rhoLeft[faceIdx] * uBarLeft + ctx.rhoRight[faceIdx] * uBarRight);

        // face-centered x-momentum density flux
        ctx.rhoUFlux[faceIdx] = 0.5 * ctx.faceArea[faceIdx] *
                                (rhoULeft * uBarLeft + rhoURight * uBarRight +
                                (ctx.pLeft[faceIdx] + ctx.pRight[faceIdx]) * ctx.faceNormalX[faceIdx] -
                                (ctx.bXLeft[faceIdx] * bBarLeft + ctx.bXRight[faceIdx] * bBarRight));
                                                             
        // face-centered y-momentum density flux
        ctx.rhoVFlux[faceIdx] = 0.5 * ctx.faceArea[faceIdx] *
                                (rhoVLeft * uBarLeft + rhoVRight * uBarRight +
                                (ctx.pLeft[faceIdx] + ctx.pRight[faceIdx]) * ctx.faceNormalY[faceIdx] -
                                (ctx.bYLeft[faceIdx] * bBarLeft + ctx.bYRight[faceIdx] * bBarRight));
        
        // face-centered z-momentum density flux
        ctx.rhoWFlux[faceIdx] = 0.5 * ctx.faceArea[faceIdx] *
                                (rhoWLeft * uBarLeft + rhoWRight * uBarRight +
                                (ctx.pLeft[faceIdx] + ctx.pRight[faceIdx]) * ctx.faceNormalZ[faceIdx] -
                                (ctx.bZLeft[faceIdx] * bBarLeft + ctx.bZRight[faceIdx] * bBarRight));
        
        // face-centered total energy density flux
        ctx.rhoEFlux[faceIdx] = 0.5 * ctx.faceArea[faceIdx] *
                                ((rhoELeft + ctx.pLeft[faceIdx]) * uBarLeft +
                                (rhoERight + ctx.pRight[faceIdx]) * uBarRight -
                                uBLeft * bBarLeft - uBarRight * bBarRight);
...
std::unique_ptr<IFluxScheme> flux_scheme_factory(Profile const& profile, ExecutionController const& execCtrl) {
    ...
    if (fluxSchemeName == FluxScheme::HLLC) {
        return std::make_unique<HLLC>();
    }
    if (fluxSchemeName == FluxScheme::KT) {
        return std::make_unique<KT>();
    }
    if (fluxSchemeName == FluxScheme::HOG) {
